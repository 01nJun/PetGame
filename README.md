...
- extends(상속)와 interface 구현 <br> <br>
  상속을 사용한 이유로는 Pet이라는 '설계도'를 만들고 모든 펫들이 물려받으면 Pet에다가 공통 기능들을 만들어 놓으면 <br>
  자동으로 물려받기 때문에 코드의 양을 줄이려고 사용했다. <br><br>
  interface는 모든 펫이 진화하는 것은 아니기 때문에 진화가 가능한 펫에게만 Evolvable이라는 '진화의 증표'를 부여하고<br> 진화가 가능한 '아기' 펫과 불가능한 펫을 구분하기 위해 사용했습니다.
